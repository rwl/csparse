...
/// clear w
int _cs_wclear(int mark, int lemax, Int32List w, int w_offset, int n) {
...
bool _diag(int i, int j, aij, other) => i != j;
...
Int32List cs_amd(int order, DZcs A) {
  DZcs C, A2, AT;
  Int32List Cp, Ci, last, W, len, nv, next, P, head, elen, degree, w, hhead, ATp, ATi;
  int lemax = 0,
      mindeg = 0,
      nel = 0;
  int d, dk, dext, e, elenk, eln, i, j, k, k1, k2, k3, jlast, ln, dense, nzmax, nvi, nvj, nvk, mark, wnvi, cnz, p, p1, p2, p3, p4, pj, pk, pk1, pk2, pn, q, n, m, t;
  ...
  cs_fkeep(C, _diag, null); // drop diagonal entries
  Cp = C.p;
  cnz = Cp[n];
  ...
  for (i = 0; i < n; i++) {
    ...
    } else if (d > dense) // node i is dense
    {
      ...
      if (head[head_offset + d] != -1) last[head[head_offset + d]] = i;
  ...
  while (nel < n) // while (selecting pivots) do
  {
    ...
    for (pk = pk1; pk < pk2; pk++) // scan2: degree update
    {
      ...
      {
        j = Ci[p];
        if ((nvj = nv[nv_offset + j]) <= 0) continue; // node j dead or in Lk
        d += nvj; // degree(i) += |j|
      ...
      for ( ; i != -1 && next[next_offset + i] != -1; i = next[next_offset + i], mark++) {
        ...
        for (j = next[next_offset + i]; j != -1; ) // compare i with all j
        {
          ...
          for (p = Cp[j] + 1; ok && p <= Cp[j] + ln - 1; p++) {
            if (w[w_offset + Ci[p]] != mark) ok = false; // compare i and j
          }
    ...
    p = pk1;
    for (pk = pk1; pk < pk2; pk++) // finalize Lk
    {
      i = Ci[pk];
      if ((nvi = -nv[nv_offset + i]) <= 0) {
        continue; // skip if i is dead
  ...
  /* Postordering */
  for (i = 0; i < n; i++) {
    Cp[i] = _CS_FLIP(Cp[i]); // fix assembly tree
  }
  for (j = 0; j <= n; j++) {
